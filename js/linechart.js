function drawLineChart(data) {
  ...
  // Draw lines and circles with tooltip
  grouped.forEach((values, key) => {
    ...
    svg.selectAll(`.circle-${key}`)
      ...
      .on("mouseover", function (event, d) {
        ...
        tooltip
          .html(`<strong>${d.state}</strong><br>${d3.timeFormat("%B %Y")(d.date)}<br>Fines: ${d3.format(",")(d.fine)}`)
      ...
      .on("click", function (event, d) {
        ...
        const filteredMonth = data.filter(e =>
  ...
  function drawMonthlyBarChart(data, date) {
    ...
    d3.select("#monthly-chart").selectAll("*").remove(); // Clear existing
    ...
    // Reuse color scale consistent with line chart
    const states = [...new Set(data.map(d => d.state))];
    const color = d3.scaleOrdinal()
      .domain(states)
      .range(d3.schemeTableau10);
    ...
    // Bars with dynamic color from state
    svg.selectAll("rect")
      .data(data)
      .enter()
      .append("rect")
